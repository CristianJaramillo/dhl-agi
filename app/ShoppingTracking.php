...
class ShoppingTracking extends Application
{
	...
	public function action()
	{
		...
		$noReferencia = (object) $this->agi->get_variable("NO_REFERENCIA");

		if($noReferencia->code == 200)
		{
			$this->agi->verbose("Ha ingresado la referencia $noReferencia->data");

			$this->agi->verbose("Preparando consulta");
			$stmt = $this->connection->prepare($this->sql);

			$this->agi->verbose("Agregando NO_REFERENCIA");
			$stmt->bindValue("noReferencia", $noReferencia->data, "string");
